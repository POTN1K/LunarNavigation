...
# for the satellites to be out of the SOI at the intersection point
alpha0 = 0.172796405  # 0.17217 [rad] is the min
# Inclination
...
ir = ir_max  # 4*np.pi/180  # [rad] => actual inclination angle of the red orbit : used as an INPUT
...
### Angles of the moon and the four satellites on inclined orbits
M = np.array([0, 0])
S3 = np.array([alpha0, ir])
S4 = np.array([-alpha0, ir])
S5 = np.array([alpha0, ig])
S6 = np.array([-alpha0, ig])
...
def moon_sat_cart_pos(sat, t):
    rc = np.zeros((len(t), 3))
    Theta = sat[0] + t*np.sqrt(mu_E/R_M**3)
    for i in range(len(t)):
        rc[i, :] = [R_M * np.cos(Theta[i]) * np.sin(Phi),
                    R_M * np.sin(Theta[i]) * np.sin(Phi),
                    R_M * np.cos(Phi)]
...
print(transf_RM(rc_S3_R[np.argmax(DS3), :], rc_M[np.argmax(DS3), :])[0])
print(rc_M[np.argmax(DS3), :])
rc_S4_Mn_max = transf_RM(rc_S4_R[np.argmax(DS4), :], rc_M[np.argmax(DS4), :])[1]
rc_S5_Mn_max = transf_RM(rc_S5_R[np.argmax(DS5), :], rc_M[np.argmax(DS5), :])[1]
rc_S6_Mn_max = transf_RM(rc_S6_R[np.argmax(DS6), :], rc_M[np.argmax(DS6), :])[1]
...
rc_S5_Mn_min = transf_RM(rc_S5_R[np.argmin(DS5), :], rc_M[np.argmin(DS5), :])[1]
rc_S6_Mn_min = transf_RM(rc_S6_R[np.argmin(DS6), :], rc_M[np.argmin(DS6), :])[1]
...
Dist_S5_min = np.sqrt(rc_S5_Mn_min[0] ** 2 + rc_S5_Mn_min[1] ** 2 + rc_S5_Mn_min[2] ** 2)
Dist_S5_max = np.sqrt(rc_S5_Mn_max[0] ** 2 + rc_S5_Mn_max[1] ** 2 + rc_S5_Mn_max[2] ** 2)
Dist_S6_min = np.sqrt(rc_S6_Mn_min[0] ** 2 + rc_S6_Mn_min[1] ** 2 + rc_S6_Mn_min[2] ** 2)
Dist_S6_max = np.sqrt(rc_S6_Mn_max[0] ** 2 + rc_S6_Mn_max[1] ** 2 + rc_S6_Mn_max[2] ** 2)
